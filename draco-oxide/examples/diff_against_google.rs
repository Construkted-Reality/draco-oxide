...
fn main() {
    ...
    eprintln!("\n{}: {} Draco primitive(s)", path, drcs.len());
...
fn inspect_unique_id_alignment(glb: &[u8]) {
        ...
        .enumerate()
    {
            ...
            .enumerate()
        {
            ...
            let Some(attrs) = ext.get("attributes").and_then(|v| v.as_object()) else {
                continue;
            };
            ...
            for a in &raw.attributes {
                eprintln!(
                    "    unique_id={} semantic={:?} dim={} type={:?}",
                    a.unique_id, a.gltf_semantic, a.dim, a.component_type
                );
            }
        }
    }
...
fn inspect_accessor(
    ...
    let mismatch = if bv_idx.is_some() {
        ...
        if actual_bv_len < expected_bytes {
            format!(
                " *** BV TOO SHORT (need {} got {}) ***",
                expected_bytes, actual_bv_len
            )
        } else {
            String::new()
        }
    ...
    eprintln!(
        ...
        bv_info,
        if min.is_some() {
            format!(" min={:?}", min.unwrap())
        } else {
            String::new()
        },
        if max.is_some() {
            format!(" max={:?}", max.unwrap())
        } else {
            String::new()
        },
        mismatch,
...
fn diff_one(drc: &[u8]) {
    ...
    eprintln!(
        ...
        if our.vertex_count == google.config.vertex_count() {
            ""
        } else {
            "MIS"
        },
    ...
    eprintln!(
        ...
        if our.index_count == google.config.index_count() {
            ""
        } else {
            "MIS"
        },
    ...
    let our_pos_attr = our
        .attributes
        .iter()
        .find(|a| a.gltf_semantic == Some("POSITION"));
    let our_norm_attr = our
        .attributes
        .iter()
        .find(|a| a.gltf_semantic == Some("NORMAL"));
    let our_uv_attr = our
        .attributes
        .iter()
        .find(|a| a.gltf_semantic == Some("TEXCOORD_0"));
    ...
    let g_uv = g_attrs
        .iter()
        .find(|a| a.dim() == 2 && a.data_type() == GAttrTy::Float32)
        .copied();

    let our_positions = our_pos_attr
        .map(|a| {
            read_f32(
    ...
    let our_pos_bins: Vec<[i32; 3]> = our_positions
        .iter()
        .map(|p| [bin(p[0]), bin(p[1]), bin(p[2])])
        .collect();
    let google_pos_bins: Vec<[i32; 3]> = google_positions
        .iter()
        .map(|p| [bin(p[0]), bin(p[1]), bin(p[2])])
        .collect();
    let our_pos_unique: std::collections::HashSet<[i32; 3]> =
        our_pos_bins.iter().copied().collect();
    let google_pos_unique: std::collections::HashSet<[i32; 3]> =
        google_pos_bins.iter().copied().collect();
    let common = our_pos_unique.intersection(&google_pos_unique).count();
    eprintln!(
        "  position bins (1cm grid): unique ours={} google={} common={}",
        our_pos_unique.len(),
        google_pos_unique.len(),
        common
    );
    ...
    for chunk in our_indices.chunks_exact(3) {
        let mut t = [
            our_pos_bins[chunk[0]],
            our_pos_bins[chunk[1]],
            our_pos_bins[chunk[2]],
        ];
        t.sort();
    ...
    for chunk in google_indices.chunks_exact(3) {
        let mut t = [
            google_pos_bins[chunk[0]],
            google_pos_bins[chunk[1]],
            google_pos_bins[chunk[2]],
        ];
        t.sort();
        *google_pos_tri.entry(t).or_insert(0) += 1;
    }
    let only_ours = our_pos_tri
        .iter()
        .filter(|(k, _)| !google_pos_tri.contains_key(*k))
        .count();
    let only_google = google_pos_tri
        .iter()
        .filter(|(k, _)| !our_pos_tri.contains_key(*k))
        .count();
    let pos_tri_match = our_pos_tri == google_pos_tri;
    eprintln!(
        "  position triangles: ours={} google={} match={} (only_ours={} only_google={})",
        our_pos_tri.len(),
        google_pos_tri.len(),
        pos_tri_match,
        only_ours,
    ...
    // mis-routing.
    if !pos_tri_match {
        let mut ours_only: Vec<&[[i32; 3]; 3]> = our_pos_tri
            .keys()
            .filter(|k| !google_pos_tri.contains_key(*k))
            .collect();
        let mut google_only: Vec<&[[i32; 3]; 3]> = google_pos_tri
            .keys()
            .filter(|k| !our_pos_tri.contains_key(*k))
            .collect();
        ours_only.sort();
        google_only.sort();
        ...
        // both ways.
        let google_set: std::collections::HashSet<[[i32; 3]; 3]> =
            google_only.iter().map(|f| **f).collect();
    ...
    let our_normals = our_norm_attr.map(|a| {
        read_f32(
            &our.data[a.offset..a.offset + a.byte_length],
            a.dim as usize,
            our.vertex_count as usize,
    ...
    let google_normals = g_norm.map(|a| {
        read_f32(
            &google.data[a.offset() as usize..a.offset() as usize + a.lenght() as usize],
            a.dim() as usize,
            google.config.vertex_count() as usize,
    ...
    let only_ours_t = our_tuples
        .iter()
        .filter(|(k, _)| !google_tuples.contains_key(*k))
        .count();
    let only_google_t = google_tuples
        .iter()
        .filter(|(k, _)| !our_tuples.contains_key(*k))
        .count();
    eprintln!(
    ...
    for ((p, n, _), c) in &our_tuples {
        *our_pn.entry((*p, *n)).or_insert(0) += c;
    }
    for ((p, n, _), c) in &google_tuples {
        *google_pn.entry((*p, *n)).or_insert(0) += c;
    }
    let pn_only_ours = our_pn
        .iter()
        .filter(|(k, _)| !google_pn.contains_key(*k))
        .count();
    let pn_only_google = google_pn
        .iter()
        .filter(|(k, _)| !our_pn.contains_key(*k))
        .count();
    eprintln!(
    ...
    for ((p, _, u), c) in &our_tuples {
        *our_pu.entry((*p, *u)).or_insert(0) += c;
    }
    for ((p, _, u), c) in &google_tuples {
        *google_pu.entry((*p, *u)).or_insert(0) += c;
    }
    let pu_only_ours = our_pu
        .iter()
        .filter(|(k, _)| !google_pu.contains_key(*k))
        .count();
    let pu_only_google = google_pu
        .iter()
        .filter(|(k, _)| !our_pu.contains_key(*k))
        .count();
    eprintln!(
    ...
    if let Some(on) = &our_normals {
        eprintln!("                    norm={:?}", on[0]);
    }
    if let Some(ou) = &our_uvs {
        eprintln!("                    uv={:?}", ou[0]);
    }
    eprintln!("  sample google[0]: pos={:?}", google_positions[0]);
    if let Some(gn) = &google_normals {
        eprintln!("                    norm={:?}", gn[0]);
    }
    if let Some(gu) = &google_uvs {
        eprintln!("                    uv={:?}", gu[0]);
    }
...
fn strip_b3dm_header(b3dm: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let r = |off: usize| {
        u32::from_le_bytes([b3dm[off], b3dm[off + 1], b3dm[off + 2], b3dm[off + 3]]) as usize
    };
